...
De acuerdo a \cite{reyes} y \index{wwwww}\cite{www} Curva 3D con las rectas: tangente normal y binormal además los planos: osculador, rectificante y normal.


\begin{prop}[www] In the definition of defn you need to use the first optional argument of  newtheorem to indicate that this environment shares the counter of the previously defined thm environment.\end{prop}
...
import geometry;
size(10cm,0);
currentcoordsys=cartesiansystem((0,0),i=(1,0),j=(0,1));
show(currentcoordsys);
pair a=(.2,.4), b=a+3*(1,0), c=rotate(90)*(b-a)/2+b;
point A=a, B=b, C=c;
...
circle c1=circle(C,length(A-B)/2);
draw(A--B--C--cycle,dashed+blue);
draw(A--B,black+2pt);
arc arc=arc(c1,208,-90);
draw(reverse(arc),Arrow);
line l=line(A, C);

point[] inter=intersectionpoints(c1,l);
dot("$T$", inter[0], 2(-.3,-1));

circle c2=circle(A,length(A-inter[0]));
arc arc2=arc(c2,-0,29);
draw(reverse(arc2),blue,Arrow);

point[] inter2=intersectionpoints(c2,line(A,B));
dot("$\phi$", inter2[1], (1,1));

distance(Label("$1$"),A,inter2[1],3mm);
distance(Label("$\frac{1}{\phi}$"),inter2[1],B,4mm);
distance(Label("$\frac{\phi}{2}$"),B,C,4mm);
distance(Label("$\frac{\phi}{2}$"),inter[0],C,-4mm);
distance(Label("$1$",align=I*dir(A--C)),A,inter[0],-3mm);
...
\begin{longtable}{ccc>{\color{blue}}c>{\color{blue}}c}
 ...
 \multicolumn{4}{c}{{\bfseries \tablename\ \thetable{} -- continua de la página anterior}} \\
	...
	$\frac{x+1}{x}=\frac{x}{1}$&$ x^2-x-1=0$     & $x=\frac{1\pm\sqrt{5}}{2}$  & $x_1=\fpeval{round((3+sqrt(5))/2,3)}$ & $x_2=\fpeval{round((1-sqrt(5))/2,3)}$\\
	$\frac{1}{x}  =\frac{x}{1-x}$&$ x^2+x-1=0 $  &  $x=\frac{-1\pm\sqrt{5}}{2}$& $x_1=\fpeval{round((-3+sqrt(5))/2,3)}$ & $x_2=\fpeval{round((1-sqrt(5))/2,3)}$\\
	$\frac{x+1}{x}=\frac{x}{1}$&$ x^2-x-1=0$     & $x=\frac{1\pm\sqrt{5}}{2}$  & $x_1=\fpeval{round((1+sqrt(5))/2,3)}$ & $x_2=\fpeval{round((1-sqrt(5))/2,3)}$\\
	$\frac{x+1}{x}=\frac{x}{1}$&$ x^2-x-1=0$     & $x=\frac{1\pm\sqrt{5}}{2}$  & $x_1=\fpeval{round((1+sqrt(5))/2,3)}$ & $x_2=\fpeval{round((1-sqrt(5))/2,3)}$\\
\end{longtable}
